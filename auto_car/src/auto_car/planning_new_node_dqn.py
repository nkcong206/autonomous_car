...
from .lib.model import DQNAgent
    ...
    def planning_thread(self):
        if self.automatic:
            ...
            if self.pl_id >= len(self.pls):
                ...
                if dis >= threshold:
                    self.done = False
                    _, _, beta = self.per.speed_streering_cal( self.yaw, self.current_position, self.pls[self.pl_id])
                    min_front , min_behind = self.per.get_state()
                    state = [dis, beta, min_front , min_behind]
                    state = np.reshape(state, [1, state_size])
                    
                    action = self.agent.act(state)
                    sp, st = self.per.step(action, min_front, min_behind)
                    ...
                    _, _, beta = self.per.speed_streering_cal( self.yaw, self.current_position, self.pls[self.pl_id])
                    min_front , min_behind = self.per.get_state()
                    next_state = [dis, beta, min_front , min_behind]
                    next_state = np.reshape(next_state, [1, state_size])
                    reward = self.per.compute_reward(next_state[0], action)
                
                    self.agent.remember(state, action, reward, next_state, self.done)
